...
SkillTrace は、スキルを使った処理（run）において、次の4つの evidence stream を取得し、それらを比較します。
...
SkillTrace は、大きく4種類の evidence を比較します。
...
### 4. Agent の実行ログ
SkillTraceは、Codex CLI、Claude Code、またはGemini CLIによって保持されている、ローカル記録（スキルの読み込み、ツール操作、検証結果など）を、プライバシー保護処理済をした上で読み取ります。

収集される実行ログは、非常に有益な情報源となり得ますし、あらゆるモードでの実行に付加情報を与えます。しかし、これらのログはエージェントが管理するものであり、バージョンによる形式の変動も大きく、スキル使用の決定的な証拠として設計されたものではないため、確実な情報源として扱いません。
...
SkillTraceは、対象とする skill ファイルの構成は、`AGENTS.md` ＋ `.agents/skills/`、または `CLAUDE.md` ＋ `.claude/skills/` であると想定しています。
...
なお、小さな LLM を使うと、MCP を経由した skill 利用の自己申告をし忘れる場合があります。その場合は、より強力な LLM で試してみてください。
...
![Skillファイルのスナップショット](/images/skilltrace-introduction/skilltrace-snapshot-1.webp =500x)
![Skillファイルの変更箇所](/images/skilltrace-introduction/skilltrace-snapshot-2.webp =500x)
...
- 実行ログの収集は現在、Codex CLI rollout files、Claude Code project-session files、および Gemini CLI chat-session files をサポートしています。
- Agent の実行ログは、クライアントがローカルで保持し、形式は安定していません。ログが欠落していたり、曖昧であったり、または形式が変更された場合でも実行には支障はありませんが、イベントが出力されない場合があります。
- `SKILL.md` へのアクセスが検知されても、それはAgent による利用目的ではなく、作業開始時にSkill の概要を集めるためかもしれません。
...
SkillTrace 自体はローカルで実行され、基本的にはリモートアクセスは必要ありません（ただし UI は Google Fonts を読み込みます）。そうは言っても、センシティブな開発情報を取得・ローカルに保存する可能性があることには注意が必要です。
...
- skill paths, operation target paths, verification categories and outcomes, agent session/client/model identifiers といった正規化された実行ログから得られる情報
- SkillTrace version、および OS プラットフォーム、CPUアーキテクチャ、Node.js のバージョン、プローブのバックエンドといったローカルランタイムのメタデータ
...
> [**人類の知識蓄積の単位は「文章」から「実行可能な作業単位」へ移行しつつあるのではないか。**](https://zenn.dev/h1deya/articles/future-of-human-knowledge-accumulation)
