...
- 自然言語で Copilot を作る（9:30）
  ...
  - あとは、口調をどうするかとか、テキストでやり取りをしていく
  - 最後にツールから「返答にあたっての、[グラウンディング（生成内容の裏付け）](https://atmarkit.itmedia.co.jp/ait/articles/2403/06/news027.html) の際のデータ・ソースは何か？」と聞かれる
  - そこで、URL を２つ返す（これらのページの内容を元に生成して返答してくれ、という意味）
  - これで画面下の「I'm Done」ボタン、続いて右上の「Create」ボタンを押せば、カスタム Copilot が出来あがる
  - その後、画面右側のフレイグラウンドでこの Copilot をテストできる
