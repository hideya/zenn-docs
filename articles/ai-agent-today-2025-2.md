...
ちなみに、上述ような、**複数の AI エージェント（LLM）やアルゴリズムを連携させて、統合的に機能させる技術や手法** のことを、最近は「**AI エージェント・オーケストレーション**」、または単に「**AI オーケストレーション**」と呼んだりもします（これも最近のバズワードの１つです）。

その場合、「AI エージェント構築プラットフォーム」は、「AI オーケストレーション・プラットフォーム」とかいう呼び方になります。

なお「AI オーケストレーション」も新しい用語なので、定義には多少ブレがあって、以下の参考文献のような、もう少し高級な定義もあります（ちなみに、その必要性をまとめた引用の後半は、とてもコンサルちっくですが、多くの要点が手際よく詰め込まれていて参考になります）：
...
「AI オーケストレーション」という言葉は、情報検索時の利便性を考えてご紹介したのですが、用語としてまだちょっと安定していない印象もあるので、以下では引き続き **「AI エージェント構築プラットフォーム」** と呼ぶことにしましょう。


次の節では、そういった「AI エージェント構築プラットフォーム」つまり、**「（狭義での）AI エージェント**（LLMエージェント や カスタムLLM と呼ばれることもある）**を組み合わせて、AI システム（広義での AI エージェント）を構築するシステム」** に関して、Microsoft や Googleといった **主要なプレーヤー達** がどのような動きをしているか、具体的に把握して、この激流の中の、各社の狙い所を見出すことを試みてみます。
...
以下ではそういった各社のプラットフォームやサービスについて、やや詳しくみていきます。
...
つまり、この Microsoft のアプローチで重要な点は、**すでに広く使われている AI チャット「Copilot」を、各種各様の AI エージェントにカスタマイズ** できるようにする、というものです。そうすることにより、今日の Copilot の使い方に慣れた多くのユーザは、同じ雰囲気で社内業務専用 AI エージェントを使い始めることができ、**社内への AI エージェント導入時の 心理的障壁 を格段に下げる** ことができます。一見何の変哲もないですが、ユーザ心理への配慮と自社の立ち位置の強みを活かした、良く考えられたアプローチだと思います。
...
あらゆる面で導入障壁を下げることを目指すというこの徹底された姿勢、そこで（ともすると技術者的には華やかさのない）「テキストでのチャット」という UI に着目したのは、本当にユーザ・ファーストの姿勢がチームで共有されていることに関心します（チーム・マネージメントをどうやってるのか興味がある）。

なお、ムービーではこの後、画面のキャンバス上で、フローチャート的な UI（PowerApps や PowerAutomate 風）を使って、ハコを足したりつなげたりして、カスタム Copilot を作るデモが続きます（17:00 くらいから）。
...
その後、いろいろと込み入った例の説明があった後、「**Copilot Studio のアーキテクチャの説明**」（31:45）として、内部ではどのようなコンポーネントがどう連携して動いているのかの説明があります（すでに長くなったので、ここではこれ以上深入りしないでおきます）。
