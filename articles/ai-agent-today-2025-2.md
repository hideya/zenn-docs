...
![robot-marching](/images/ai-agent-today-2025-2/robots-discussing.png =600x)
...
**「[オープンソース（オープンパラメータ）LLM」の世界を切り開き、AI コミュニティと LLM の発展に絶大な貢献をした](https://huggingface.co/models) Meta** ですが、
...
![meta-llama-stack](/images/ai-agent-today-2025-2/meta-llama-stack.png =500x)
...
![meta-llama-stack-code](/images/ai-agent-today-2025-2/meta-llama-stack-code.png =500x)
...
つまりは、**極端に言えば、「コンピュータを使う業務をしている人の代行をする AI エージェント」が作れちゃう**ことを意味します。
...
1. まずデモを起動すると、Linux が立ち上がった画面がブラウザに見えます（Docker内でLinux動かしてVNCでブラウザに画面を飛ばしています）：

2. 画面左側のチャット入力から「Firefoxを開いて」と指示すると、
   - 「Firefoxを開くために、まずデスクトップのスクリーンショットを撮って、Firefoxアイコンの位置を確認します。」
   - 「Firefoxのアイコンを見つけました。アイコンをクリックするために、マウスを移動させてクリックします。」
   - 「Firefoxが起動しました。指示通りスタートアップウィザードは無視して、必要な場合はアドレスバーに直接URLを入力できる状態になっています。」
3. 「cnn.com のページを表示して」
   - 「CNNのウェブサイトを表示するために、アドレスバーをクリックしてURLを入力します。」
   - 「CNNのウェブサイトが表示されました。ページが完全に読み込まれるまで少し待つ必要があるかもしれません。…」
   - 「CNNのウェブサイトが正常に表示されました。メインページにはニュース記事、ヘッドライン、動画などが表示されています。必要に応じて、特定の記事やセクションを閲覧することができます。」
4. 「日本に関するニュースを表示して」
   - 「CNNの日本関連ニュースを探すために、上部のトップニュース一覧にある "Japan sinkhole" の項目をクリックします。」
   - 記事が表示されなかった様子…
   - 「別の方法で日本関連のニュースを探してみましょう。上部メニューの "World" をクリックして、そこから日本のニュースを探します。」
   - 「World セクションで日本に関する記事が見つかりました。シンクホール（陥没）に関する記事があります。この記事をクリックして詳細を見てみましょう。」
   - 「記事が表示されました。この記事は東京で発生したシンクホール（陥没）に関するもので、トラックの運転手が巻き込まれ、周辺住民が不安を感じているという内容です。記事の詳細を読むために、ページをスクロールダウンしてみましょう。…」
